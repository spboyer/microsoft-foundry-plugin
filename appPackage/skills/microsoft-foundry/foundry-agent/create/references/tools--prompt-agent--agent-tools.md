...
|------|------------------------|-------------|-----------|
| Code Interpreter | `CodeInterpreterTool` | No | [tool-code-interpreter.md](./tools--prompt-agent--tool-code-interpreter.md) |
| Function calling (client-side) | `FunctionTool` | No | [tool-function-calling.md](./tools--prompt-agent--tool-function-calling.md) |
| File Search | `FileSearchTool` | No (vector store required) | [tool-file-search.md](./tools--prompt-agent--tool-file-search.md) |
| Web Search (preview) | `WebSearchPreviewTool` | No (basic Bing); **Yes** for Grounding with Bing Custom Search — the connection scopes grounding to specific domains | [tool-web-search.md](./tools--prompt-agent--tool-web-search.md) |
| Bing Grounding | `BingGroundingTool` | Yes (Bing) | [tool-bing-grounding.md](./tools--prompt-agent--tool-bing-grounding.md) |
| Azure AI Search | `AzureAISearchTool` | Yes (Search) | [tool-azure-ai-search.md](./tools--prompt-agent--tool-azure-ai-search.md) |
| MCP server (remote) | `MCPTool` | Optional (none / static key / project MI / OAuth) | [tool-mcp.md](./tools--prompt-agent--tool-mcp.md); toolbox attach via [foundry-tool-catalog.md](foundry-tool-catalog.md) |
| OpenAPI tool | (n/a as a single class) | Conditional — `connection` auth requires `project_connection_id`; **`managed_identity` auth does NOT** (the project MI is used directly with an `audience`) | [tool-openapi.md](./tools--prompt-agent--tool-openapi.md) |
| Agent-to-Agent (A2A) | (n/a as a single class) | Optional | [tool-a2a.md](./tools--prompt-agent--tool-a2a.md) |
| Agent Memory | `MemorySearchPreviewTool` | Yes (project MI + embedding model) | [tool-memory.md](./tools--prompt-agent--tool-memory.md) |
| **Work IQ (preview)** | `WorkIQPreviewTool` | Yes (Work IQ BYO-Entra-app OAuth connection) | [tool-work-iq.md](./tools--prompt-agent--tool-work-iq.md) |
| **Fabric IQ (preview)** | `FabricIQPreviewTool` | Yes (Fabric IQ Entra-app OAuth or managed-OAuth connection) | [tool-fabric-iq.md](./tools--prompt-agent--tool-fabric-iq.md) |
