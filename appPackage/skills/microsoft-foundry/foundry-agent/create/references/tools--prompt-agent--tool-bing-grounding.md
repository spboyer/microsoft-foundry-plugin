...
Access real-time web information via Bing Search. Unlike the [Web Search tool](./tools--prompt-agent--tool-web-search.md) (which works out of the box), Bing Grounding requires a dedicated Bing resource and a project connection.

> ⚠️ **Warning:** Use the [Web Search tool](./tools--prompt-agent--tool-web-search.md) as the default for web search. Only use Bing Grounding when the user **explicitly** requests Grounding with Bing Search or Grounding with Bing Custom Search.
...
- [Web Search Tool (default)](./tools--prompt-agent--tool-web-search.md)
